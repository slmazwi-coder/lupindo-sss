    ...
    <section className="section-pad" style={{ background: '#fffdf5' }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-end justify-between mb-10 gap-4 flex-wrap">
          ...
          <Link to="/news" className="text-sm font-semibold flex items-center gap-1" style={{ color: '#166534' }}
            onMouseEnter={e => { e.currentTarget.style.color = '#C8A400' }}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {news.map(item => (
            <article key={item.id} className="card flex flex-col">
              ...
              <h3 className="font-display font-bold text-lg mb-2 leading-tight" style={{ color: '#166534' }}>
                {item.title}
              </h3>
              <p className="text-sm flex-1 mb-4" style={{ color: '#374151' }}>
                {item.excerpt}
              </p>
              <span className="text-sm font-semibold flex items-center gap-1" style={{ color: '#C8A400' }}>
